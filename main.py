...
# ----------------- EXISTING ROUTES (Goals/Savings/AI) -----------------
...
def get_prediction(goal_id: int):
    return {"prediction": "Keep depositing consistently to track timeline!"}
@app.post("/api/v1/smart-split")
def smart_split(req: SmartSplitRequest):
    return {"error": "AI connected but needs more active targets."}
@app.get("/api/v1/monitor/{goal_id}")
def daily_monitor(goal_id: int):
    return {"status": "Market is stable today."}
...
def find_dupe(goal_id: int):
    return {"error": "Could not find alternative right now."}
@app.get("/api/v1/stock-check/{goal_id}")
def check_stock(goal_id: int):
    return {"status": "UNKNOWN", "message": "Verify manually."}
...
def get_appliances():
    ...
    cursor.execute("SELECT * FROM appliances ORDER BY watts DESC")
    apps = cursor.fetchall()
    
    # Calculate monthly theoretical consumption for each (Watts * Hours * 30 days / 1000)
    for app in apps:
        app['monthly_units'] = round((app['watts'] * app['daily_hours'] * 30) / 1000, 2)
...
def add_appliance(app: ApplianceCreate):
    conn = get_db_connection(); cursor = conn.cursor()
    cursor.execute("INSERT INTO appliances (appliance_name, watts, daily_hours) VALUES (%s, %s, %s)", (app.appliance_name, app.watts, app.daily_hours))
    conn.commit(); cursor.close(); conn.close()
...
def energy_coach(units: float, bill: float, rate: float):
    ...
    theoretical_units = sum(((a['watts'] * a['daily_hours'] * 30) / 1000) for a in apps)
    app_list = ", ".join([f"{a['appliance_name']} ({a['watts']}W, {a['daily_hours']}h/day)" for a in apps])
    
    prompt = f"""
    The user lives in Kerala, India and receives electricity from KSEB. 
    They have logged these appliances: {app_list}.
    Based on their listed appliances, they SHOULD consume {round(theoretical_units, 1)} units/month.
    However, their actual meter reading shows they consumed {units} units this month, costing ₹{bill} (Rate: ₹{rate}/unit).
    
    Analyze the gap between theoretical and actual units. Is there a phantom load? Are ACs/Fridges working overtime due to Kerala humidity? 
    Give exactly 3 high-impact, actionable bullet points to reduce this specific KSEB bill next month. 
    ...
    try:
        ...
        return {"tips": tips, "theoretical_units": round(theoretical_units, 1)}
    except Exception as e:
        return {"tips": ["Turn off phantom loads (devices left plugged in).", "Set AC temperature to 24°C to save 6% power per degree.", "Switch older tube lights and bulbs to energy-efficient LEDs."], "theoretical_units": round(theoretical_units, 1)}